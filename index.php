...
if (preg_match($global_config['check_module'], $module_name)) {
	...
	$string_tmp = explode('?',end($array_url));
	$strings = $string_tmp[0];
	$string = str_replace('.html','',$strings);
	
	
	if (substr($string, 0, 5) == 'page-') 
	{
			$page = intval(substr($strings, 5));
			$alias_mod = str_replace('/','',$array_url[$count-1]);
	}
	else
	{
		$alias_mod = str_replace('/','',$strings);
	...
	if (!isset($site_mods[$module_name]) and !empty($alias_mod))
	{
		...
		$modnamenew = $db->query(" SELECT module_data FROM " . NV_PREFIXLANG . "_modules WHERE module_file = 'news'")->fetchColumn();
		...
		if($array_alias['id'])
		{
			...
			////////////////////////////////////
			$global_parent_id_news = $db->query(" SELECT parentid FROM " . NV_PREFIXLANG . "_" . $modnamenew . "_cat WHERE catid = " . $array_alias['catid_alias'])->fetchColumn();
			
			if(!empty($global_parent_id_news))
			{
				$global_parent_news = $db->query("SELECT catid, title, alias AS link FROM " . NV_PREFIXLANG . "_" . $modnamenew . "_cat WHERE catid = " . $global_parent_id_news)->fetchAll();
				
			}
			$global_title_news = $db->query(" SELECT title FROM " . NV_PREFIXLANG . "_" . $modnamenew . "_cat WHERE catid = " . $array_alias['catid_alias'])->fetchColumn();			
			$global_title_id_news = $db->query(" SELECT catid FROM " . NV_PREFIXLANG . "_" . $modnamenew . "_cat WHERE catid = " . $array_alias['catid_alias'])->fetchColumn();			
			
			$global_link_news = $db->query(" SELECT alias FROM " . NV_PREFIXLANG . "_" . $modnamenew . "_cat WHERE catid = " . $array_alias['catid_alias'])->fetchColumn();			
